...
namespace Interface
{
    public class NetworkClient : Client
    {
        ...
        {
            Node.Send(RequestType.ProgressionAnswer, new string[]{position.ToString(), name, JsonConvert.SerializeObject(choice), answer.ToString(), progression is null ? "" : progression.ToString()});
        }
