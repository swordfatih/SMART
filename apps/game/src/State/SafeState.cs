            ...
            else if (action == 1)
            {
                ...
                switch (choice.Answers[answer])
                {
                    ...
                    case "Message":
                        ...
                        break;
                    case "Gardien":
                        communication = new ChoiceCommunication(player, direction, new("Le gardien est-il devant toi ?", new() { "Oui", "Non" }, player.Position));
                        break;
                    case "Progression":
                        communication = new ProgressionCommunication(player, direction, new("Veux-tu partager ta progression ?", new() { "Oui", "Non" }, player.Position));
                        break;
                    case "Opinion":
                        communication = new ChoiceCommunication(player, direction, new("Ton avis sur ton autre voisin ?", new() { "Malveillance max", "Un bon", "Je ne sais pas" }, player.Position));
                        break;
                    default:
                        break;
                }
